...
- **LLM wiring** lives in `Services/LLMWiring.swift`; until PR #5 merges it returns nil (the pipeline runs `PassthroughCleaner` and `FakeSummarizer`, as the CLI does without an endpoint) and the Test button reports that the client is not part of the build. The LLM settings save URL, model, context tokens and the key already.
