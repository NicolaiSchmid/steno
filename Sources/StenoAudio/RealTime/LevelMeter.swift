...
struct LevelMeter: Sendable, Equatable {
  ...
  var linearPeak: Float { peakValue }

  /// The window so far, without resetting.
  var current: LaneLevel {
...
final class LevelSlot: @unchecked Sendable {
  ...
  @inline(__always)
  func publish(mic: LaneLevel, system: LaneLevel?) {
  ...
  var currentGeneration: Int { generation.load(ordering: .acquiring) }

  var levels: LaneLevels {
    LaneLevels(
      mic: LaneLevel(
