...
/// verification. CryptoKit where it exists; a portable implementation where
/// it does not (the Linux container used for development), so the same
/// bytes hash identically everywhere.
public enum ContentHash {
  ...
  public static func sha256(contentsOf url: URL) throws -> Data {
  ...
  static func hash(_ message: Data) -> Data {
    ...
    for chunk in stride(from: 0, to: bytes.count, by: 64) {
      for i in 0..<16 {
        let base = chunk + 4 * i
        w[i] =
          UInt32(bytes[base]) << 24 | UInt32(bytes[base + 1]) << 16 | UInt32(bytes[base + 2]) << 8
          | UInt32(bytes[base + 3])
      }
      for i in 16..<64 {
