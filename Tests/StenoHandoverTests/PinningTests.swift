  ...
  @Suite struct PinningTests {
    ...
    @Test func oneFlippedFingerprintBitFailsTheHandshakeBeforeAnyRequest() async throws {
      try await TestService.run { test in
        ...
        flipped[0] ^= 0x01
        let client = try test.client(fingerprint: flipped)
    ...
    @Test func anotherMintedCertificateIsRejected() async throws {
      try await TestService.run { test in
        let other = try MintedIdentity.mint(commonName: "Steno on Another Mac")
        let client = try test.client(fingerprint: other.fingerprint)
    ...
    @Test func thePhonesEvaluatorComputesTheFingerprintTheQRCarries() async throws {
      ...
      try await TestService.run { test in
        ...
        #expect(
          identity.fingerprint == HandoverIdentity.fingerprint(ofDER: identity.certificateDER))
        ...
        #expect(
          !PinnedTrustEvaluator.evaluate(
            presented, pinnedFingerprint: scanned.fingerprint.prefix(31))
        )
    ...
    @Test func plaintextHTTPToTheListenerNeverReachesTheRouter() async throws {
      ...
      try await TestService.run { test in
        guard case .listening(let port) = test.service.state else {
          Issue.record("the service is not listening")
          return
        }
