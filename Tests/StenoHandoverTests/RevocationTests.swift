...
@Suite struct RevocationTests {
  ...
  @Test func revokeAndUnpairDropTheUploadAndMakeEveryBearerRoute401() async throws {
    ...
    #expect(try await test.store.handoverReceipt(recordingID: a.id) == nil, "the receipt cascades")
    ...
    let unpair = try await b.phone.client.request("DELETE", "/v1/pairing", headers: b.phone.bearer)
